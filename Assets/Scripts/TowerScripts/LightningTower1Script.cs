...
public class LightningTower1Script : MonoBehaviour
{
    ...
    public void Fire(Transform t)
    {
        ...
        {
            GameObject LightningBall = Instantiate(LightningBallPrefab);
            LightningBall.transform.SetPositionAndRotation(pivot.transform.position, pivot.transform.rotation);
            LightningScript ts = LightningBall.GetComponent<LightningScript>();
            ts.target = t.gameObject;
            ts.speed = speed;
            ts.damage = damage;
            ts.range = transform.Find("Range").localScale.x / 2.0f;
            ts.maxDistance = maxProjectileDistance;
            timer = 0f;
